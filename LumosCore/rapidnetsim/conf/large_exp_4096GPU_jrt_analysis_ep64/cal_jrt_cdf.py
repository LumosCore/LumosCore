...
lumoscore_25_6t = load_csv_get_beta(f'lumoscore_tau_2/task_time.log')
clos = load_csv_get_beta(f'ele/task_time.log')
clos_4tier_25_6t = load_csv_get_beta(f'ele_26T/task_time.log')
clos_3tier_25_6t = load_csv_get_beta(f'ele_26T/task_time.log')

oct_list = np.array(get_completion_time(best, task_nums))
lumoscore_2tau_times = get_completion_time(lumoscore_2tau, task_nums)
lumoscore_1tau_times = get_completion_time(lumoscore_1tau, task_nums)
lumoscore_25_6t_times = get_completion_time(lumoscore_25_6t, task_nums)
clos_times = get_completion_time(clos, task_nums)
clos_4tier_25_6t_times = get_completion_time(clos_4tier_25_6t, task_nums)
clos_3tier_25_6t_times = get_completion_time(clos_3tier_25_6t, task_nums)
...
plt.ylim(bottom=0.5)  # Set y-axis to start from 0.5
plt.xlabel('Slow down of Job Running Time(s)', fontsize=12)  # Add x-axis label with font size
plt.yticks([0.5, 0.6, 0.7, 0.8, 0.9, 1.0], fontsize=10)  # Set y-ticks and font size
...
for i, key in enumerate(jrt_data.keys()):
    ax.plot([], [], color=colors[i], linestyle=line_styles[i], label=key, linewidth=2)

ax.legend(ncol=3, loc='center', fontsize=10)
