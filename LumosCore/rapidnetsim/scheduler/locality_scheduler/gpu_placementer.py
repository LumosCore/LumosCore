...
class GpuPlacementer:
    def __init__(self,  spine_switch_num, leaf_switch_num, spine_switch_port_num, leaf_switch_port_num, server_num, oxc_num = 32):
        ...
        if 'banned_spine_num' in Simulator.CONF_DICT and Simulator.CONF_DICT['banned_spine_num'] != '':
            ...
            ban_spine_num = 0
        self.banned_spine_list = [i for i in range(ban_spine_num)]
        # self.spine_resource_manager_ = spine_resource_manager.SpineSwitchManager(self.spine_num, self.port_per_spine, [])
    ...
    def schedule(self, gpu_num, job_id, sim_time, queued_jobs, spine_strategy_mode ="gpu_first", oxc_need_reconfig = False):
        ...
        print("debug choose_group_in_spine_result",choose_group_in_spine_result)
        ...
        while(temp_require_leaf_num<=self.leaf_num and temp_require_leaf_num<=gpu_num):
            ...
            if can_find_gpu:
                allocate_success, job_leaf_to_spine_map, job_oxc_leaf_spine_map, leaf_occupy_gpu_num_map, spine_occupy_port_num_map, temp_need_spine_migration, leaf_remain_gpu_num_map = self.connection_manager_.find_valid_network_new(gpu_num, res_leaf_occupy_gpu_num_map, spine_remain_empt_port_list, temp_require_leaf_num, temp_require_spine_num, job_id, leaf_remain_empt_gpu_list, self.banned_spine_list)
            ...
            if allocate_success:
                ...
                spine_port_num = 0
                tmp_choosed_spine_index_list_gur = []
                for chosen_spine_id in spine_occupy_port_num_map:
                    tmp_choosed_spine_index_list_gur.append(chosen_spine_id)
                ...
                print("debug choosed_spine_index_list_gur ", tmp_choosed_spine_index_list_gur)
