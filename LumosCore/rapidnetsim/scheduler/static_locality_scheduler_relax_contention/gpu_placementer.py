...
class StaticPlacementerRelax:
    def __init__(self, spine_switch_num, leaf_switch_num, spine_switch_port_num, leaf_switch_port_num, server_num,
                 oxc_num=32):
        ...
        print(f"Cluster Info: server_num: {server_num}, leaf_num: {leaf_switch_num}, gpu_num: {self.gpu_num}")
        self.server_resource_manager_ = ServerResourceManager(server_num, self.gpu_per_server, self.leaf_num)
        self.leaf_resource_manager_ = LeafResourceManager(self.leaf_num, self.gpu_per_leaf)
        self.connection_manager_ = ConnectionManager(self.gpu_num, self.server_num, self.leaf_num, self.spine_num)
    ...
    def schedule(self, gpu_num, job_id, sim_time, queued_jobs):
        ...
        for temp_leaf_id in range(self.leaf_num):
            ...
            valid_server_num = 0
            for temp_server_num in range(int(temp_leaf_id * self.gpu_per_leaf / self.gpu_per_server),
                                         int((1 + temp_leaf_id) * self.gpu_per_leaf / self.gpu_per_server), 1):
                if self.server_resource_manager_.server_list[
                    temp_server_num].remain_gpu_num() >= require_gpu_num_in_server:
                    valid_server_num += 1
            if valid_server_num >= require_server_num:
                potential_leaf_list.append(
                    [temp_leaf_id, sum(self.leaf_resource_manager_.leaf_list[temp_leaf_id].leaf_group)])
        potential_leaf_list.sort(key=lambda x: (x[1]))
        if len(potential_leaf_list) > 0:
            temp_leaf_id = potential_leaf_list[0][0]
            ...
            with open('queue_length.txt', 'a') as f2:
                f2.write(f"{len(queued_jobs)},{sim_time}\n")
            time_end = time.perf_counter()
            time_sum = time_end - time_start
            Simulator.SCHEDULER_TIME_COST[job_id] = 0
            with open('schedule_time_cost.txt', 'a') as f3:
                f3.write(f"{job_id},{time_sum}\n")
        ...
        print("debug fuck temp_require_leaf_num", temp_require_leaf_num)
        ...
        for temp_server_id in range(self.server_num):
            ...
            leaf_remain_empt_gpu_list[temp_leaf_id] += sum(
                self.server_resource_manager_.server_list[temp_server_id].gpu_group)
        allocate_success, allocation_link_mapping, leaf_occupy_gpu_num_map, job_allocated_leaf_spine_link, contention_link = self.connection_manager_.find_valid_gpu_for_no_pow2_tas_releax_iter(
            gpu_num, leaf_remain_empt_server_list, temp_require_leaf_num, leaf_remain_empt_gpu_list, self.link_weight)
        if allocate_success:
            chosen_gpu_list = []
            for chosen_leaf_id in leaf_occupy_gpu_num_map:
                chosen_gpu_list.extend(self.server_resource_manager_.choose_gpu_in_one_leaf(chosen_leaf_id,
                                                                                            leaf_occupy_gpu_num_map[
                                                                                                chosen_leaf_id]))
                self.leaf_resource_manager_.leaf_list[chosen_leaf_id].update_leaf_group_with_required_num(
                    leaf_occupy_gpu_num_map[chosen_leaf_id])
            for output_gpu_index in chosen_gpu_list:
            ...
            f3.write("\n")
            f3.close()
            self.task_contention_map[job_id] = contention_link
            return True, True, chosen_gpu_list, allocation_link_mapping, contention_link, None, None
            # else:
