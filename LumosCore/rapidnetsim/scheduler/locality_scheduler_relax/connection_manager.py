...
class ConnectionManager:
    ...
    def find_valid_gpu_for_specific_spine(self, require_gpu_num, require_spine_id, server_remain_gpuNum_map,job_allocated_oxc_spine_link, used_spine_port_num_pair, leaf_remain_empt_server_list):
        ...
        xnum_i = {}
        for it in name_list_x_i:
            xnum_i[it] = m.addVar(vtype=gurobipy.GRB.INTEGER, lb=0, ub=self.gpu_per_server,name='xnum_i')
        ...
        for i in range(self.server_num):
            m.addConstr(xnum_i[str(i)]>=x_i[str(i)])
        
        for i in range(self.server_num):
            m.addConstr(xnum_i[str(i)]== gurobipy.quicksum( z_i_j[str(i)+'_'+str(j)] for j in range(self.oxc_num)) )
        ...
        m.addConstr(gurobipy.quicksum(xnum_i[str(i)] for i in range(self.server_num)) == require_gpu_num)
        # 目标函数
        leaf_num_k = {}
        ...
        if m.status == gurobipy.GRB.Status.OPTIMAL:
            ...
            assert(round(gurobipy.quicksum(xnum_i[str(i)] for i in range(self.server_num)).getValue()) == require_gpu_num)
            ...
            for it in name_list_x_i:
                server_occupy_gpuNum_map[int(it)] = int(round(xnum_i_solution[it]))
    ...
    def find_valid_gpu_for_no_pow2_task(self, require_gpu_size, leaf_remain_empt_server_list, spine_remain_empt_port_list, require_leaf_num, require_spine_num, job_id, leaf_remain_empt_gpu_list):
        ...
        assert remain_chosen_gpu_num == 0
        leaf_index_sub_leaf_index_list_map = {}
        spine_max_sub_spine_num_map = {}
        sub_leaf_require_gpu_num = max(1,int(require_spine_num))
        ...
        for leaf_id in range(len(leaf_remain_empt_server_list)):
            num_sub_leaf = int(leaf_remain_empt_gpu_list[leaf_id]/sub_leaf_require_gpu_num)
            # print(num_sub_leaf, int(leaf_remain_empt_server_list[leaf_id]/sub_leaf_require_server_num))
            #assert num_sub_leaf == int(leaf_remain_empt_server_list[leaf_id]/sub_leaf_require_server_num)
        ...
        for it in name_list_s_i:
            s_i[it] = model.addVar(vtype=gurobipy.GRB.INTEGER, lb=0, ub=leaf_remain_empt_gpu_list[int(it)],name='s_i')
            x_r_i[it] = model.addVar(vtype=gurobipy.GRB.INTEGER, lb=0, ub=leaf_remain_empt_gpu_list[int(it)],name='x_r_i')
            temp_x_r_i[it] = model.addVar(vtype=gurobipy.GRB.INTEGER, lb=0, ub=int(leaf_remain_empt_gpu_list[int(it)]),name='x_r_i')
        ...
        for i in range(self.leaf_num):
            if leaf_index_sub_leaf_index_list_map[i]!=[]:
                model.addConstr(gurobipy.quicksum(x_i_a[str(i)+'_'+str(a)]*require_spine_num for a in leaf_index_sub_leaf_index_list_map[i]) == s_i[str(i)] )
        ...
        for i in range(self.leaf_num):
            if leaf_index_sub_leaf_index_list_map[i]!=[]:
                model.addConstr(  x_r_i[str(i)] == temp_x_r_i[str(i)] )
