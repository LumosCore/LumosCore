import copy
import math
import rapidnetsim.scheduler.locality_scheduler_relax.utils as utils
...
class ServerResourceManager:
    ...
    def choose_gpu_in_one_leaf(self, leaf_id, need_group_size):
        ...
        can_deuce = False
        need_server_num = 1
        while(is_power_of_2(need_group_size) and need_server_num<=self.server_per_leaf):
        ...
        if not can_deuce:
            if is_power_of_2(need_group_size):
                ...
                chosen_gpu_num = 0
                relate_server_list.sort(key=lambda x: x.remain_gpu_num(), reverse=True)
                remain_to_choose_gpu = need_group_size
                while(remain_to_choose_gpu>0):
                    to_chosen_num = min(remain_to_choose_gpu, relate_server_list[0].remain_gpu_num())
                    temp_chosen_gpu_list = self.server_list[relate_server_list[0].server_id].occupy_gpu_with_required_num(to_chosen_num)[1]
                    gpu_to_choose_list.extend(temp_chosen_gpu_list)
                    chosen_gpu_num += len(temp_chosen_gpu_list)
                    remain_to_choose_gpu -= len(temp_chosen_gpu_list)
                    del relate_server_list[0]
                assert chosen_gpu_num == need_group_size
        # 首先记录(server_id, group_size),按group size从大到小排列，因为need_group_size肯定大于最大的group_size，因此可以推断出所需的group大小
        # serverid_groupsize_pair_list = []
        # for potentional_server in relate_server_list:
