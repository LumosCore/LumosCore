...
class GPUPlacement:
    def __init__(self):
        ...
        self.banned_gpu_status = np.zeros((infra.pod_num, self.leaf_per_pod, infra.server_per_leaf * self.TP),
                                          dtype=int)  # 记录gpu是否被故障
    ...
    def occupy_gpu(self, pod_id, used_server_num, task_id):
        ...
        for tmp_leaf_id in range(self.leaf_per_pod):
            tmp_leaf_remain_server_num = self.server_per_leaf - np.sum(
                self.pod_gpu_status[pod_id, tmp_leaf_id, :]) // self.TP - np.sum(
                self.banned_gpu_status[pod_id, tmp_leaf_id, :]) // self.TP
        ...
        for tmp_leaf_id in range(self.leaf_per_pod):
            x_i[tmp_leaf_id] = model.addVar(vtype=gurobipy.GRB.INTEGER, lb=0, ub=self.server_per_leaf - np.sum(
                self.pod_gpu_status[pod_id, tmp_leaf_id, :]) // self.TP - np.sum(
                self.banned_gpu_status[pod_id, tmp_leaf_id, :]) // self.TP, name='x_i')
            ...
            for tmp_leaf_id in range(self.leaf_per_pod):
                if round(x_i_solution[tmp_leaf_id]) > 0:
                    ...
                    while has_chosen_gpu < x_i_solution[tmp_leaf_id] * self.TP:
                        if self.pod_gpu_status[pod_id, tmp_leaf_id, tmp_flag] == 1 or self.banned_gpu_status[
                            pod_id, tmp_leaf_id, tmp_flag] == 1:
                            tmp_flag += 1
                        else:
    ...
    def occupy_resource(self, task_require_server_num, EP_size, task_id):
        ...
        for pod_id in range(self.pod_num):
            model.addConstr(
                x_i[pod_id] <= self.server_num_per_pod - self.busy_server_per_pod[pod_id] - self.banned_server_per_pod[
                    pod_id])
        ...
        model.addConstr(100 * gurobipy.quicksum(used_i[pod_id] for pod_id in range(self.pod_num))
                        + gurobipy.quicksum(used_i[pod_id] * (
                self.server_num_per_pod - self.busy_server_per_pod[pod_id] - self.banned_server_per_pod[pod_id])
    ...
    def release_resource(self, task_id):
        ...
        self.task_occupy_gpu_map[task_id] = np.zeros((self.pod_num, self.leaf_per_pod, self.server_per_leaf * self.TP),
                                                     dtype=int)
    ...
    def random_fail_gpu(self, failure_id):
        ...
        new_banned_gpu_status = np.zeros((self.pod_num, self.leaf_per_pod, self.server_per_leaf * self.TP), dtype=int)
        new_banned_server_per_pod = np.zeros(self.pod_num, dtype=int)
        for pod_id in range(self.pod_num):
            for local_leaf_id in range(self.leaf_per_pod):
                for local_gpu_id in range(self.server_per_leaf * self.TP):
                    if self.pod_gpu_status[pod_id, local_leaf_id, local_gpu_id] == 1 and self.banned_gpu_status[
                        pod_id, local_leaf_id, local_gpu_id] == 0:
                        potentional_gpu_list.append(pod_id * self.leaf_per_pod * self.server_per_leaf * self.TP
                                                    + local_leaf_id * self.server_per_leaf * self.TP + local_gpu_id)
        ...
        chosen_pod_id = random_banned_gpu // (self.leaf_per_pod * self.server_per_leaf * self.TP)
        chosen_local_leaf_id = (random_banned_gpu % (self.leaf_per_pod * self.server_per_leaf * self.TP)) // (
                self.server_per_leaf * self.TP)
        chosen_local_server_id = ((random_banned_gpu % (self.leaf_per_pod * self.server_per_leaf * self.TP)) % (
                self.server_per_leaf * self.TP)) // self.TP
        for potention_gpu_id in potentional_gpu_list:
            cur_pod_id = potention_gpu_id // (self.leaf_per_pod * self.server_per_leaf * self.TP)
            cur_local_leaf_id = (potention_gpu_id % (self.leaf_per_pod * self.server_per_leaf * self.TP)) // (
                    self.server_per_leaf * self.TP)
            cur_local_server_id = ((potention_gpu_id % (self.leaf_per_pod * self.server_per_leaf * self.TP)) % (
                    self.server_per_leaf * self.TP)) // self.TP
            cur_local_gpu_id = (potention_gpu_id % (self.leaf_per_pod * self.server_per_leaf * self.TP)) % (
                    self.server_per_leaf * self.TP)
            if (cur_pod_id == chosen_pod_id) and (cur_local_leaf_id == chosen_local_leaf_id) and (
                    chosen_local_server_id == cur_local_server_id):
        ...
        print("debug failure pod", chosen_pod_id, chosen_task_id)
        # 更新受影响的pod
