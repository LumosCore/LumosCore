...
def test_routing():
    conf_path = ('/mnt/d/Code/PyCharm/rapidnetsim-moe/large_exp_4096GPU_beta_analysis_ecmp_railop/'
                 'beta_2000/lumoscore_tau_1_26T/exp.ini')
    ...
    Simulator.CONF_DICT['ocs_reconfiguration'] = 'no'
    # Simulator.CONF_DICT['find_next_hop_method'] = 'balance'
    Simulator.create_infrastructure()
    Simulator.load_scheduler()
    Simulator.CONF_DICT['ocs_reconfiguration'] = 'yes'
    server_num = Simulator.get_infrastructure().server_num
    for i in range(4096, 4096+512):
        print(sorted(Simulator.intra_pod_down_table[i].keys()))
    ...
    # print(sorted(Simulator.intra_/pod_down_table[4352].keys()))
