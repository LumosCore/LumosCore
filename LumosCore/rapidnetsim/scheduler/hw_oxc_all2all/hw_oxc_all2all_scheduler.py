...
class HwOxcAll2allScheduler:
    def __init__(self, NIC_num_in_a_server, NIC_num) -> None:
        ...
        for i in range(NIC_num, NIC_num + NIC_num_in_a_server):
            ...
            if task_occupied_NIC_num >= NIC_num_in_a_server:
                ...
                for switch_id, leisure_num in self._switch_leisure_NIC_num_map.items():
                    if leisure_num == NIC_num_in_a_server:
                        tmp_NIC_record += self.get_NIC_list_in_switch(switch_id, NIC_num_in_a_server, self._NIC_num, NIC_num_in_a_server)
                ...
                if residual_need_num > 0:
                    for (switch_id, leisure_num) in switch_leisure_NIC_num_map_list:
                        if leisure_num < NIC_num_in_a_server and leisure_num > residual_need_num:
                            tmp_NIC_record += self.get_NIC_list_in_switch(switch_id, NIC_num_in_a_server, self._NIC_num, residual_need_num)
                    ...
                    if residual_need_num > 0:
                        for (switch_id, leisure_num) in switch_leisure_NIC_num_map_list:
                            ...
                            tmp_NIC_record += self.get_NIC_list_in_switch(switch_id, NIC_num_in_a_server, self._NIC_num, leisure_num)
    ...
    def get_NIC_list_in_switch(self, switch_id, NIC_num_in_a_server, NIC_num, need_num):
        if need_num == NIC_num_in_a_server:
            ...
            print("Bug: get_NIC_list_in_switch need_num > NIC_num_in_a_server")
            return